...
make_helper(cmp_a2moffs_v) {
	return (suffix == 'l' ? cmp_a2moffs_l(eip) : cmp_a2moffs_w(eip));
}

make_helper(cmp_moffs2a_v) {
	return (suffix == 'l' ? cmp_moffs2a_l(eip) : cmp_moffs2a_w(eip));
}
