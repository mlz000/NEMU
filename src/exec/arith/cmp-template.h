...
#include "cpu/modrm.h"
void concat(setflag1_, SUFFIX) (DATA_TYPE t1,DATA_TYPE t2) {//t1 - t2
	DATA_TYPE t = t1 - t2;
...
make_helper(concat(cmp_r2rm_, SUFFIX)) {
	...
	m.val = instr_fetch(eip + 1, 1);
	if(m.mod == 3) {
		concat(setflag1_, SUFFIX) (REG(m.R_M), REG(m.reg));
		...
		swaddr_t addr;
		int len = read_ModR_M(eip + 1, &addr);
		concat(setflag1_, SUFFIX) (addr, REG(m.reg));
...
make_helper(concat(cmp_rm2r_, SUFFIX)) {
	...
	m.val = instr_fetch(eip + 1, 1);
	if(m.mod == 3) {
		concat(setflag1_, SUFFIX) (REG(m.reg), REG(m.R_M));
		...
		swaddr_t addr;
		int len = read_ModR_M(eip + 1, &addr);
		concat(setflag1_, SUFFIX) (REG(m.reg), MEM_R(addr));
